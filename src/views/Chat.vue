              ...
              <transition-group :name="transitionName">
                <div class="absolute flex items-center justify-center w-full h-full z-10 bg-gray-800" key="loading" v-if="isLoading">
                  <div class="flex justify-center items-center flex-col">
                    <div class="loader animate-spin ease-linear rounded-full border-4 border-t-4 border-gray-700 border-t-lime-600 h-10 w-10"></div>
                    <div>
                ...
                <div class="absolute w-full h-full" key="chats" v-if="chats.length && location === 'chats'">
                  <div class="w-full overflow-auto h-full flex flex-col scrollbar scrollbar-thin scrollbar-thumb-gray-900 scrollbar-track-gray-800">
                    <transition-group name="message">
                      <div @click="loadChat(chat)" :class="[ chat === chats[0] ? 'mt-5' : 'mt-2', chat._id === currentChat.id ? 'bg-white-900 border-transparent' : 'bg-gray-100']" v-for="chat in chats" :key="chat._id" class="hover:bg-gray-600 cursor-pointer border-transparent transition items-center grid grid-cols-6 mb-2 mx-5 p-5 max-w-full rounded-lg">
                ...
                <div class="w-full h-full absolute" key="no-chats" v-if="!chats.length && location === 'chats'">
                  <div class="text-center -mt-10 flex flex-col justify-center items-center h-full text-gray-600 text-sm">
                    <ExclamationCircleIcon class="w-14 h-14 mb-5 text-lime-600" />
                    <div class="text-gray-600 text-sm">No chats yet</div>
                    <div class="text-gray-600 text-sm">Start chatting with your friends</div>
                    <button class="hover:border-blue-300 border-transparent border mt-5 text-lime-600 rounded-lg px-4 py-2" @click="setLocation('new')">Start Chatting</button>
                  </div>
                </div>
            ...
            <div v-if="currentChat.visible" key="chat-view" class="chatContainer absolute w-full h-full">
              <div class="relative w-full h-full flex flex-col">
                <div class="chatHeader flex items-center px-8 py-3 border-b border-blue-300">
                  ...
                  <div v-show="currentChat.typing" id="wave" class="ml-10 flex">
                    ...
                    <div v-show="!mq.mdMinus">
                      <span class="dot inline-block w-1.5 h-1.5 rounded-full mr-1 bg-lime-600"></span>
                      <span class="dot inline-block w-1.5 h-1.5 delay-100 rounded-full mr-1 bg-lime-600"></span>
                ...
                <div class="relative overflow-hidden w-full h-full opacity-50">
                  <div ref="msgContainer" :class="[ currentChat.allowScrollAnimation ? 'scroll-smooth' : '' ]" class="overflow-auto flex relative h-full scrollbar scrollbar-thin scrollbar-thumb-gray-900 scrollbar-track-white-200">
                ...
                <div class="chatFooter flex items-center px-8 py-3 border-t border-t-lime-600">
                  <div>
            ...
            <div v-if="!currentChat.visible" key="no-chat-view" class="w-full h-full flex absolute items-center justify-center">
              <div class="flex-col items-center justify-center text-center">
                <svg class="ml-auto mr-auto" width="200" viewBox="0 0 594 437" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M65 252C65 228.252 84.2518 209 108 209H395C418.748 209 438 228.252 438 252V365C438 388.748 418.748 408 395 408H108C84.2518 408 65 388.748 65 365V252Z" class="fill-current text-lime-600" />
                ...
                <div >
                  ...
                  <p class="text-gray-600 mt-5 text-sm">
                    Or start a new chat by clicking <span @click="setLocation('new')" class="text-lime-600 cursor-pointer">here</span>
                  </p>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async waitForDBLoad() {
      ...
      await this.loadChats();
      this.rtm = new RtManager(this.ipfsNode.pubsub, 'devSend123');
