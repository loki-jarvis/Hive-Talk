...
export default {
  ...
  methods: {
    ...
    async connect(user) {
      ...
      let id1 = await this.ipfsNode.id()
      const id2=id1.id
      console.log(id2.toString())
      let validip4 = new Multiaddr('/ip4/172.31.207.209/tcp/4001/p2p/12D3KooWExixqRY39ULucKr7Wv7jjLNTpGwQkcThfw3MBKRk7ntv');
      const resp = await node.bootstrap.add(validip4);
      console.log(resp);
      const controller = await Database.createInstance(this.ipfsNode,this.$options={id:id2.toString(),offline:false});
